<?php

return [
    'log' => 'Log in',
    'username' => 'username',
    'password' => 'password',
    'text_no_account' => 'dont have an account yet ? ',
    'text_create_one' => 'create one here',
    'creation_success' => 'Account created successfuly',
    'nav' => [
        'student' => 'Student',
        'create_student' => 'Create Student',
        'forum' => 'Forum',
        'publish' => 'Publish',
        'documents' => 'Documents',
        'logout' => 'Logout',
    ],
    'forum' => [
        'title' => 'Forum',
        'add_comment' => 'Add a comment',
        'comment'  => 'Comment',
    ],

    'forum_create' => [
        'title' => 'Create a post',
        'title_post' => 'Title',
        'title_post_placeholder' => 'Title of your post',
        'content_post' => 'Content',
        'content_post_placeholder' => 'Content of your post',
        'submit' => 'Submit',
    ],
    'student' => [
        'title' => 'Student List',
        'add_student' => 'Add a student',
        
        'view_profile' => 'View Profile',
    ],

    'student_create' =>[
        'title' => 'Create a student',
        'name' => 'Name',
        'name_placeholder' => 'Enter the name of the student',
        'address' => 'Address',
        'address_placeholder' => 'Address of the student',
        'phone' => 'Phone',
        'phone_placeholder' => 'Phone of the student',
        'birthday' => 'Birthday',
        'birthday_placeholder' => 'Birthday of the student',
        'town' => 'Town',
        'email' => 'Email',
        'email_placeholder' => 'Email of the student',
         'creation' => 'Creation',
        'submit' => 'Submit',
    ],

    'student_create_modal' =>[
        'body' => 'Are you sure you want to create this student ?',
        'close' => 'Close',
        'submit' => 'Submit',
    ],

    'student_edit' =>
    [
        'title' => 'Edit a student',

    ],

    'student_edit_modal' =>[
        'update' => 'Update',
        'body' => 'Are you sure you want to edit this student ?',
        'close' => 'Close',
        'submit' => 'Submit',
    ],
  

    




    
];

